namespace CommunityCraft {
	...
	void RaycastUpdate() {
		...
		for (GLuint aadvanced = 1; aadvanced <= 5; aadvanced++) {
			scurrentPos = glm::normalize(vec3(srx, -sry, -srz));
		...
		if (instance->GetButton(GLFW_MOUSE_BUTTON_LEFT) && allowed && !ppress) {
			SetBlock(currentPos, airID);
		}
		if (instance->GetButton(GLFW_MOUSE_BUTTON_RIGHT) && allowed && !ppressr) {
			if (coords.y > currentPos.y) {
				if (IsAirW({ currentPos.x, currentPos.y + 1, currentPos.z })) {
					SetBlock({ currentPos.x, currentPos.y + 1, currentPos.z }, stoneID);
				}
			}
			else if (coords.y < currentPos.y) {
				if (IsAirW({ currentPos.x, currentPos.y - 1, currentPos.z })) {
					SetBlock({ currentPos.x, currentPos.y - 1, currentPos.z }, stoneID);
				}
			}
