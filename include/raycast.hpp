namespace OpenCraft {
	...
	void RaycastUpdate() {
		...
		if (instance->GetButton(GLFW_MOUSE_BUTTON_LEFT) && allowed && !ppress) {
			universe->SetBlock(currentPos, airID);
		}
		if (instance->GetButton(GLFW_MOUSE_BUTTON_RIGHT) && allowed && !ppressr) {
			if (coordinates.y > currentPos.y) {
				if (universe->IsAir({ currentPos.x, currentPos.y + 1, currentPos.z })) {
					universe->SetBlock({ currentPos.x, currentPos.y + 1, currentPos.z }, stoneID);
				}
			}
