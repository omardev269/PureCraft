...
const auto renderDistance = 5;
#include <ElementHierarchy/universe.hpp>
namespace OpenCraft {
	Universe* universe;
}
#include <raycast.hpp>
...
void Update(){
	...
	if (OpenCraft::instance->GetKey(GLFW_KEY_W)) {
		...
		if (!freefly) {
			if (!OpenCraft::universe->IsAir(-OpenCraft::location) || !OpenCraft::universe->IsAir({ -OpenCraft::location.x, -OpenCraft::location.y - 1, -OpenCraft::location.z }) || !OpenCraft::universe->IsAir({ -OpenCraft::location.x, -OpenCraft::location.y - 2, -OpenCraft::location.z })) {
				OpenCraft::location.z -= (10.0f * OpenCraft::instance->deltaTime) * cos(radians(OpenCraft::rotation.y));
				OpenCraft::location.x += (10.0f * OpenCraft::instance->deltaTime) * sin(radians(OpenCraft::rotation.y));
			}
		}
	...
	OpenCraft::transform = Transform::Projection(OpenCraft::instance->windowSize, 55.0f, 0.05f, 500.0f);
	...
	OpenCraft::universe->UniverseUpdate();
	OpenCraft::transform = Transform::Projection(OpenCraft::instance->windowSize, 55.0f, 0.05f, 500.0f);
	...
	MCGL::UI::Print("PureCraft Build 2");
...
int main() {
	OpenCraft::instance = new Instance({ 800,600 }, "PureCraft Build 2", false, false);
	...
	OpenCraft::mesh = new GameObject(OpenCraft::dumb, 36 * 256 * 32 * 32 * 32, 6);
	OpenCraft::mesh->amountOfVertices = 0;
	OpenCraft::mesh->SetUsage(1, 3, 2);
	OpenCraft::crosshair = new GameObject(OpenCraft::crosshairVertices, 6, 6);
	OpenCraft::crosshair->SetUsage(1, 3, 2);
	OpenCraft::universe = new OpenCraft::Universe(static_cast<unsigned>(clock()));
	OpenCraft::instance->Start(Update);
	delete OpenCraft::universe;
